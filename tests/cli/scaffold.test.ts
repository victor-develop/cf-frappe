...
`
  ...
  it("generates missing D1 migration files from app metadata", async () => {
    ...
    expect(stdout.text()).toContain("Wrote migrations/0011_doctype_customer_v2_indexes.sql (1 statements)");
    const generated = await readFile(join(target, "migrations/0011_doctype_customer_v2_indexes.sql"), "utf8");
  ...
  it("uses stable core migration filenames when generating into an empty directory", async () => {
    ...
    expect(stdout.text()).toContain("Wrote fresh-migrations/0008_cf_frappe_fold_snapshots.sql");
    expect(stdout.text()).toContain("Wrote fresh-migrations/0009_doctype_task_v1_indexes.sql");
