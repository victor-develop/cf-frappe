...
describe("cf-frappe CLI remote doctypes", () => {
  ...
  it("gets remote DocType metadata through the generated metadata API", async () => {
    ...
    expect(stdout.text()).toContain('- title text [required,mandatoryDependsOn,unique,noCopy,allowOnSubmit,fetchFrom=project.title,fetchIfEmpty,list,search] - Title help "Human-readable task title."');
