...
Tenant customization overlays can be inspected and changed without opening Desk:
...
npx cf-frappe reports list --url https://your-worker.example --header-env Authorization=CF_FRAPPE_AUTH
...
npx cf-frappe doctypes list --url https://your-worker.example --header-env Authorization=CF_FRAPPE_AUTH
npx cf-frappe doctypes get --url https://your-worker.example --doctype Task --header-env Authorization=CF_FRAPPE_AUTH
npx cf-frappe doctypes list-view --url https://your-worker.example --doctype Task --header-env Authorization=CF_FRAPPE_AUTH
npx cf-frappe search --url https://your-worker.example --query launch --limit 10 --header-env Authorization=CF_FRAPPE_AUTH
npx cf-frappe link-options --url https://your-worker.example --doctype Task --field project --query apollo --limit 10 --header-env Authorization=CF_FRAPPE_AUTH
...
export const Task = defineDocType({
  ...
  formView: {
    sections: [
      ...
      { heading: "Website", columns: 1, fields: ["route", "published"] },
...
export const TaskUpdatesWebView = defineWebView({
  name: "Task Updates",
  label: "Task Updates",
  module: "Desk",
  description: "Published starter task updates rendered from permissioned projections.",
  doctype: "Task",
  routeField: "route",
  titleField: "title",
...
const STARTER_TASK_SEED_RECORDS = [
  {
    ...
    route: "review-generated-desk-workspace",
    published: true,
    priority: "High",
    workflow_state: "Open",
  ...
  },
  {
    ...
    route: "deploy-to-cloudflare-workers",
    published: true,
    priority: "Medium",
