...
Browser scripts can also build generated Desk navigation targets without duplicating route encoding: `window.cfFrappe.desk.listUrl(doctype, options)` returns the filtered/ordered list route, `window.cfFrappe.desk.newUrl(doctype)` returns `/desk/:doctype/new`, `window.cfFrappe.desk.formUrl(doctype, name)` returns `/desk/:doctype/:name`, and `workspaceUrl`, `dashboardUrl`, `reportUrl`, `reportPdfUrl`, `reportBuilderUrl`, `reportBuilderPdfUrl`, `printUrl`, `printPdfUrl`, `filesUrl`, `notificationsUrl`, plus `admin*Url` helpers cover the generated workspace, dashboard, report, saved report-builder, print/PDF, file-manager, notification-inbox, and admin pages.
