...
- event-sourced saved report definitions for per-user report-builder presets with extracted definition-codec/replay event modeling plus generated HTTP, Desk, browser, and remote CLI report-builder APIs
...
- event-sourced tenant role catalog with bounded event helper modeling plus generated HTTP, Desk, browser-helper, and remote CLI administration
...
- Cloudflare Queue/Cron background job primitives with event-sourced runtime schedule definitions and overrides plus bounded event helper modeling
...
- event-sourced per-user notification inboxes with bounded event helper modeling plus tenant notification rules for document commit events, with generated HTTP management, Desk browser helpers, remote inbox CLI operations, Desk read/dismiss workflows, and optional transactional email delivery through a Cloudflare Email Sending adapter
- event-sourced document delivery outbox with bounded event helper modeling for durable notification, realtime, and email after-commit intents, with idempotent enqueue, drain consumer, claim, retry, and delivery markers
...
`field` and `provided` strategies use caller data, while `uuid` uses the configured id generator. A `series` strategy plans an internal `__NamingSeries` event stream advance per tenant, DocType, and pattern and commits that advance in the same multi-stream batch as the document create event. Explicit `name` values are rejected for series-named DocTypes so metadata remains the naming authority. That keeps the counter independent of projections without consuming a number when the document create fails; Cloudflare Durable Object command routing sends series creates through one shared aggregate key for the pattern, and direct D1 commits still use stream expected-version checks and retry on counter conflicts.

Scalar fields can also declare `unique: true`. Document commands reserve new values and release old values in internal `__UniqueValues` streams in the same multi-stream batch as the document create, update, or delete event, and skip empty values. This keeps uniqueness in the event-sourced write model instead of depending on stale projections or adapter-side lookups.
...
`SavedListFilterService` stores per-user list filters as append-only events in a tenant/DocType/owner stream, validates every saved filter against the DocType list-filter metadata, and delegates replay, label policy, owner projection, sorting, payload-kind selection, and append-version reads to the bounded saved-list-filter event model. The generated resource API exposes `GET`, `POST`, and `DELETE` routes under `/api/resource/:doctype/saved-filters`, and `/api/resource/:doctype?saved_filter=<id>` applies a saved filter through the normal `QueryService.listDocumentsForView(...)` path.
...
When `documentDeliveryOutbox.emailNotificationDeliveryQueue` is configured, the Worker drain job queues email notification delivery messages instead of sending them inline, so provider retries stay on the normal Queue job path.
...
Current suite: 2787 tests across 235 Vitest files covering app manifests, metadata-defined workspaces, dashboards, Calendar views, and Kanban boards including new-document, dashboard, Calendar, and Kanban workspace shortcuts, remote workspace metadata CLI reads, report-chart dashboard cards, direct document aggregate dashboard cards, dashboard indicator rules, and metric-card drilldowns, event-sourced custom field overlays with extracted event helper modeling, HTTP/Desk/CLI administration, event-sourced field property overrides with extracted event helper modeling, HTTP/Desk/CLI administration, Desk browser helpers including customization-overlay metadata read aliases, and Worker metadata composition, event-sourced runtime workflow definitions with extracted event helper modeling and HTTP/Desk/CLI administration, Desk browser helpers including metadata read aliases, and Worker metadata composition, event-sourced notification rules with extracted event helper modeling, HTTP/Desk/CLI administration, metadata-validated post-commit conditions, inbox/email channel selection, extracted email outbox fold/claim/append event modeling with bounded event helper modeling, EmailSender-port transactional delivery, Cloudflare Email Sending adapter coverage, Worker routing, after-commit delivery, durable document delivery outbox intent recording with bounded event helper modeling and extracted fold/selector event modeling, drain consumer/job coverage, Worker queue notification/realtime/email/retry drain coverage, Worker/Queue/Cron starter wiring, Wrangler binding-type smoke coverage, and Desk browser helpers, custom fields plus parent, child table, and nested child-table custom fields applied to runtime command validation, metadata APIs, remote DocType metadata CLI reads, workspace metadata client helpers, generated form/list/child-table views, model-declared conditional required, read-only, hidden, print-hidden, empty-value-print-hidden, and placeholder-rendered field behavior, resource filters including nested compound expressions, saved filters, metadata-aware list ordering, bounded resource CSV exports, metadata-driven CSV import templates, command-bound CSV imports with browser helpers, and generated Desk CSV import and bulk action forms plus Desk document/workspace/report/dashboard/calendar/print/file content/file manager/notification/admin navigation helpers and client helpers preserving list context, metadata-driven global search with a generated Desk search page, Desk search client helpers, and remote search CLI coverage, link-option metadata client helpers, remote link-option CLI coverage, dashboard metadata client helpers, dashboard metadata/card execution, Calendar metadata/view execution with compound filters, Kanban metadata/board execution with compound filters, Web Form published and login-required filtering, public route, and success URL rendering, Web Form Website presentation rendering, Website Settings navigation including Web Form and Web View references, Web View safe-route filtering, pagination, metadata ordering, metadata flat filters, and compound filter expressions, themed rendering, Web View and website metadata browser helpers, and generated Desk dashboard, Calendar, and Kanban pages through Worker routing, HTTP writes, and Desk forms/lists, generated custom-field HTTP and Desk administration, custom-field client helpers, field-property client helpers, workflow-definition client helpers, app-declared data patches/backfills, queued data-patch apply, rollback, and rollback-retry jobs, Desk queue controls for data-patch apply/rollback/rollback-retry enqueue with batch and single-patch delivery fields, failed data-patch retry controls, failed rollback retry controls, data-patch rollback planning and execution, data-patch admin APIs, data-patch client helpers, Desk administration, remote CLI status/plan/rollback-plan/apply/rollback/enqueue/rollback-enqueue/retry/rollback-retry-enqueue operations, remote custom-field list/save/disable CLI operations, remote field-property list/save/clear CLI operations, remote workflow get/save/clear CLI operations, remote report list/get/run/export CLI operations, remote report-builder list/get/create/update/delete/run/export CLI operations, remote dashboard list/get/run CLI operations, remote Calendar list/get/run CLI operations, remote Kanban list/get/run CLI operations, remote print-formats list/get/letterheads/letterhead CLI reads plus rendered document HTML/PDF downloads, remote print-settings get/update CLI operations, remote role list/get/create/describe/enable/disable CLI operations, remote user account get/create/password/roles/provider-sync/enable/disable CLI operations, remote user profile get/update CLI operations, remote audit search/deleted-recovery CLI operations, remote notification inbox list/read/dismiss CLI operations, remote notification-rule list/get/save/enable/disable/clear CLI operations, remote resource list/get/create/update/delete/lifecycle/workflow/domain-command/duplicate/amend/bulk/timeline/comment/activity/assignment/tag/follower/share/saved-filter/CSV CLI operations, remote user-permission list/allow/revoke CLI operations, remote file get/upload/download/preview-download/transform-download/rendition-download/list/update/bulk-update/bulk-delete/rendition/delete CLI operations, client scripts, Desk browser APIs, auth/session/recovery/account-management/provider/profile client helpers including profile metadata read aliases, audit search/deleted-recovery client helpers, role catalog client helpers including metadata read aliases, account-management and provider-sync client helpers, user-permission client helpers including policy metadata read aliases, job history/schedule client helpers, filtered Desk job schedule operator actions, print metadata/document/PDF/settings client helpers, generated print-settings Desk administration with bounded event helper modeling, report/report-builder PDF client helpers, selected-document bulk resource client helpers, document-history read client helpers, file metadata/upload/direct-upload/multipart-upload/transform/rendition client helpers, notification inbox/read-state and notification-rule client helpers including single-rule reads, enable/disable toggle saves, and automation metadata read aliases, document collaboration client helpers, notification-rule event helper modeling, assignment rule event helper modeling, user-permission event helper modeling, document-share event helper modeling, document-collaboration event helper modeling, document-command event helper modeling, document-lifecycle bounded event helper modeling and event-map registration, Desk form event hooks, Desk field controls and user feedback helpers, schema, permissions, signed sessions, Cloudflare Access JWT actor resolution and event-sourced account auto-sync, generic OIDC JWT actor resolution and event-sourced account auto-sync, OIDC group-role helpers and Okta/Auth0/Google Workspace provider claim presets, generic event-sourced auth-provider account sync, event-sourced role catalog administration with bounded event helper modeling and remote CLI coverage, catalog-backed account role validation, event-sourced user accounts with bounded event helper modeling and remote CLI coverage, event-sourced user profiles and profile preferences with bounded event helper modeling and remote CLI coverage, password reset/email verification token flows, login/logout/me APIs, generated Desk account/profile administration, account-version session revalidation across HTTP and realtime auth, audit redaction and filtering for account, profile, role, field-property, workflow-definition, notification-rule, and share events, user permissions with centralized grant-change decisions, document shares, durable per-user notification inboxes with centralized access policy and bounded event helper modeling and rule-driven inbox delivery with extracted inbox fold/order/invariant event modeling, events, registry, services, naming series, workflow helpers, document lifecycle, document duplication and amendment, extracted document field mutation policy for readonly/allow-on-submit/unset/child-origin/table-copy rules, extracted document collaboration event naming and policy for comment/activity/assignment/tag/follower/share normalization, comment/activity/assignment/tag/follower event planning, share/revoke event planning, idempotent collection changes, and delegated-share checks, extracted document lifecycle event payload, create-projection, committed snapshot projection, saved-event id and first-event lookup, after-commit completion, and live-fold guard helpers, extracted document hook context, validation data, and after-commit context helpers, extracted document collaboration event payload helpers, extracted document command and share event payload helpers, extracted unique-value reservation, reserve/release event planning, write decision, owner lookup decision, release decision, reservation projection, and release projection policy, extracted document naming and naming-series event planning policy, extracted document bulk policy, extracted file bulk delete and metadata update entry command policies, extracted document reference policy, extracted document fetch-from policy, extracted document link validation policy, extracted document access policy including DocType action, document action, shared command access, user-permission access, share-management access, and shared-permission lookup decisions, extracted document query presentation policy including projected-document read, link-option candidate, global-search candidate, and projection page scan decisions, extracted document query input policy, extracted document command policy including domain-command execution, workflow-transition, create/update lifecycle, status-change, delete lifecycle, and duplicate/amend copy planning, extracted document tenant and bulk command shaping policy, multi-stream unique reservation plus document commits, event-sourced document delivery outbox claim/retry/deliver transitions plus drain job and Worker queue notification/realtime/email/retry drain coverage plus Worker/Queue/Cron starter coverage plus Wrangler binding-type smoke coverage, event-level document field unsets for rename/remove data patches, generated workflow actions with extracted command-event naming, generated bulk document deletion/lifecycle/workflow actions, document timelines and diffs, activity feed entries, admin audit search with centralized tenant access decisions and deleted recovery, comments, assignments, tags, followers, saved user filters with extracted fold/label/sort and filter-merge event modeling, saved report definitions with extracted definition-codec/replay event modeling, saved report HTTP and Desk report-builder APIs, metadata-driven Desk saved report builder filter presets including not-equals plus numeric/date/datetime same-field ranges, summaries/groups/formula columns/charts, Desk arbitrary-depth formula field/literal/nested visual operand controls, nested report formula expressions, metadata-configured form/list views, child table validation, metadata-validated system-field, pattern, presence, range, membership, compound expression, and operator-aware list filters, generated arbitrary-depth point-and-click compound condition builders, filter-builder metadata, print formats, event-sourced global print settings, print layout metadata, print templates, print letterheads, reports, custom row-provider report sources, typed report filters including between/not-between ranges, bounded report filter expression trees across HTTP, saved-report, custom-row-provider, Desk browser helper paths, and visual Desk report-builder controls, report ordering controls, report formula columns, report summaries, report group row caps, report charts, report exports, Desk report printouts, document/report PDF renderer routes and the Cloudflare Browser Rendering PDF adapter, jobs, worker pool metadata and batch-lane execution, durable job execution history, retry administration, scheduler administration with event-sourced runtime schedule definition and enable/disable/pause-until/reset override controls plus centralized job schedule configured/runtime summary projection, files, semantic upload document-data policy, centralized transformed file result policy, centralized ignored cleanup failure policy, centralized dashboard pagination query policy, centralized dashboard result and direct-upload capability shaping policy, centralized upload, download, and prepared-upload result shaping policy, centralized generated and downloaded rendition result shaping policy, centralized generated-rendition reuse storage policy, centralized generated-rendition reuse result policy, centralized generated-rendition reuse decision policy, centralized generated-rendition reuse head-read planning policy, centralized generated-rendition reuse object-existence policy, centralized generated-rendition completion manifest policy, centralized generated-rendition completion result policy, centralized generated-rendition failure recovery policy, centralized generated-rendition failure-recovery suppression policy, centralized generated-rendition failure cleanup policy, centralized generated-rendition failure cleanup planning policy, centralized object scan planning policy, centralized infected scan-failure selection policy, centralized transform-overlay document-read planning policy, centralized upload scan-failure cleanup planning policy, centralized upload scan-failure decision policy, centralized delete storage cleanup planning policy, centralized delete finalization planning policy, centralized generated-rendition failure manifest policy, centralized bulk success and result shaping policy, centralized bulk delete outcome aggregation policy, centralized bulk metadata outcome aggregation policy, centralized visible file dashboard entry policy, centralized readable file dashboard candidate and entry policy, centralized file dashboard batch-limit policy, centralized file dashboard scan planning policy, centralized file dashboard system actor policy, centralized upload data command policy, centralized multipart abort delete command policy, centralized multipart abort planning policy, centralized file document create command policy, centralized upload storage command policy, centralized buffered upload storage planning policy, centralized buffered upload failure cleanup planning policy, centralized multipart storage command policy, centralized multipart part upload planning policy, centralized multipart completion object planning policy, centralized multipart completion object read planning policy, centralized completed multipart object head-read planning policy, centralized direct-upload object head-read planning policy, centralized downloaded object read planning policy, centralized downloaded rendition object read planning policy, centralized multipart completion snapshot selection policy, centralized delete-requested snapshot selection policy, centralized file transformer command policy, centralized downloaded transform command policy, centralized transform-overlay source read policy, centralized upload completion command, version, and execute-command policy, centralized upload completion planning policy, centralized multipart part record command and execute-command policy, centralized multipart completion-start command and execute-command policy, centralized delete-request command and execute-command policy, centralized rendition manifest command policy, centralized metadata update command and execute-command policy, centralized buffered-upload create command policy, centralized buffered upload create planning policy, centralized direct-upload create command policy, centralized direct upload reservation planning policy, centralized multipart-upload create command policy, centralized multipart upload reservation planning policy, centralized multipart upload reservation cleanup planning policy, centralized rendition reservation command policy, centralized generated-rendition reservation planning policy, centralized generated-rendition storage put-command policy, centralized rendition result command-name and manifest execute-command policy, centralized delete command and execute-command policy, centralized command tenant and upload-default policy, centralized file command option policy, centralized file attachment validation policy, centralized attachment validation planning policy, centralized file attachment option policy, centralized file privacy, transform-overlay resolution, and transform-overlay option policy, centralized missing-object storage error policy, direct upload reservations/finalization, generated direct-upload Desk form enhancement, storage-level multipart upload primitives, event-sourced multipart upload orchestration and HTTP APIs, browser chunked multipart upload orchestration, file scan hooks, file metadata dashboard filters and update APIs, permissioned inline file previews, bounded image transform routes over a transformer port and Cloudflare Images adapter, styled text-watermark and image-overlay source options through an injected transformer port, event-sourced persisted image renditions, bounded bulk file deletion and metadata updates, Desk record attachment panels and filtered file manager workflows, generated admin navigation, field-level document merge planning, server-side deterministic document merge apply, generated stale-form merge save, realtime topic fan-out, redacted per-user room notifications, permissioned presence snapshots, generated realtime-updated Desk presence panels with stale-document warnings and live field-edit indicators, presence-panel merge and shared-draft apply actions including child-table patches, and Desk helpers, transient document field-edit and shared-draft patch collaboration messages, bounded durable realtime replay, parsed Desk realtime subscriptions, shared D1 event writer coverage, D1/in-memory adapters including data-patch changed-row, data-patch journal JSON, job-execution JSON, event-stream JSON, projection JSON, Durable Object replay JSON, and Durable Object websocket JSON evidence validation, HTTP API, generated Desk UI, Durable Object command routing, Worker routing, Queue/Cron/R2 integration, D1 schema planning/projection-index retirement/migration/application of data-patch journals, and CLI starter scaffolding/package-manager/dependency/app-registry/migration-file/seed-data-patch generation plus Cloudflare Access and OIDC starter auth-mode wiring and Access setup plan/apply commands.

The latest architecture slice moves naming-series allocation, unique-value reservations, unique-value releases, and the document event into one `DocumentStore.commitBatch` for create, update, and delete commands. `document-atomic-commit-policy` now owns auxiliary batch entry and projection shaping, so `DocumentService` keeps command I/O orchestration while atomic multi-stream commit composition lives behind a narrow policy. The focused regression tests prove a failed document create no longer consumes a naming-series value and a failed release batch no longer commits the document update.

The previous architecture slice moves OIDC and Cloudflare Access JWT optional claim-shape validation into access policy so HTTP auth adapters keep JWT verification and JWKS orchestration while the auth capability owns provider claim invariants, building on the prior Cloudflare Access synced-account actor resolution policy extraction and the prior Cloudflare Access account-sync claim projection policy extraction and the prior Cloudflare Access claims-to-actor mapping policy extraction and the prior OIDC account-sync allow gating policy extraction and the prior OIDC account-sync claim projection policy extraction and the prior OIDC claims-to-actor mapping policy extraction and the prior OIDC token-source configuration policy extraction and the prior OIDC provider role-list, claim-name, and hosted-domain normalization policy extraction and the prior rule API single-entry lookup policy extraction and the prior job-schedule runtime Worker Cron Trigger configuration policy extraction and the prior job-schedule capability resource availability policy extraction and the prior document-delivery outbox drain job availability policy extraction and the prior email-notification job availability policy extraction and the prior data-patch job availability policy extraction and the prior job-schedule lookup policy extraction and the prior report provider lookup policy extraction and the prior user-profile account lookup policy extraction and the prior document-delivery outbox lookup policy extraction and the prior user-notification lookup policy extraction and the prior custom-field disable lookup policy extraction and the prior saved-report/list-filter lookup policy extraction and the prior job execution lookup policy extraction and the prior Web View route/item lookup policy extraction and the prior Web Page route lookup policy extraction and the prior Website Settings home-page resolution policy extraction and the prior document query access policy extraction and the prior Website Settings access policy extraction and the prior report access policy extraction and the prior saved-report access policy extraction and the prior saved-list-filter access policy extraction and the prior print access policy extraction and the prior Web Page read-access policy extraction and the prior Web View read-access policy extraction and the prior Web Form access policy extraction and the prior Kanban read-access policy extraction and the prior Calendar read-access policy extraction and the prior dashboard read-access policy extraction and the prior data-patch access policy extraction and the prior job-schedule capability policy extraction and the prior job-retry lookup policy extraction and the prior job-history lookup policy extraction and the prior user-notification record policy extraction and the prior user-notification read/dismiss policy extraction and the prior user-account change policy extraction and the prior saved-list-filter command policy extraction and the prior saved-report command policy extraction and the prior custom-field change policy extraction and the prior field-property change policy extraction and the prior workflow change policy extraction and the prior patch change policy extraction and the prior notification-rule change policy extraction and the prior assignment-rule change policy extraction and the prior role change policy extraction and the prior saved-list-filter merge policy extraction and the prior user permission grant-change policy extraction, the prior user notification access policy extraction, the prior audit access policy extraction, the prior job schedule summary policy extraction, the prior report run/export policy extraction, the prior print view/access policy extraction and the prior custom-field table/reference guard policy extraction and the prior field-property override normalization policy extraction and the prior document delivery outbox consumer policy extraction and the prior job queue dispatch policy extraction and the prior shared concurrency policy extraction and the prior shared access-error policy extraction and the prior user-account recovery policy helper extraction and the prior job-schedule policy helper extraction and the prior email-notification service policy extraction and the prior document-import policy extraction and the prior audit policy extraction and the prior document-history assignment, tag, and follower projection extraction and the prior website-settings, web-form, web-view, kanban, calendar, custom-field, field-property, print-settings, notification-rule, assignment-rule, workflow, role, user-permission, and user-profile policy extractions, building on previous slices that shared data-patch checksum mismatch policy across runner, service, D1, and in-memory logs while preserving applied-versus-recorded error wording, shared data-patch journal policy across D1 and in-memory logs so retry, rollback-retry, rollback-claim, checksum, and apply-unavailable errors use one status boundary, extracted data-patch journal retry and rollback claimability policy from D1 data-patch-log I/O and covered retry, rollback-retry, and rollback-claim status guards directly, extracted D1 data-patch journal row/result serde from data-patch-log I/O and covered direct row projection, rollback result JSON, and invalid-result guards, extracted D1 job execution query planning from job-execution-log I/O and covered bound list-filter SQL planning with default limits, extracted D1 projection query planning from projection-store I/O and covered system updatedAt ordering without JSON path extraction, centralized print-settings state event payload-kind matching through core print-settings helpers, custom-field state event payload-kind matching through core custom-field helpers, field-property override state event payload-kind matching through core field-property override helpers, notification-rule state event payload-kind matching through core notification-rule helpers, assignment-rule state event payload-kind matching through core assignment-rule helpers, document-lifecycle event payload-kind matching through `domain-events`, document-delivery outbox event payload-kind matching through `domain-events`, job-schedule event payload-kind matching through `domain-events`, user-notification inbox event payload-kind matching through `domain-events`, user-profile event payload-kind matching through `domain-events`, print-settings event payload-kind matching through `domain-events`, role-catalog event payload-kind matching through `domain-events`, user-account event payload-kind matching through `domain-events`, email-notification outbox event payload-kind matching through `domain-events`, notification-rule event payload-kind matching through `domain-events`, workflow-definition event payload-kind matching through `domain-events`, assignment-rule event payload-kind matching through `domain-events`, document-command event payload-kind matching through `domain-events`, document-collaboration event payload-kind matching through `domain-events`, document-share event payload-kind matching through `domain-events`, saved-report event payload-kind matching through `domain-events`, saved-list-filter event payload-kind matching through `domain-events`, user-permission event payload-kind matching through `domain-events`, and field-property event payload-kind matching through `domain-events`, custom-field event payload-kind matching through `domain-events`, document-delivery outbox source-event payload-kind validation in `domain-events`, document-history timeline event-kind projection in `domain-events`, audit deleted-event recovery selection in `domain-events`, notification-rule email event-label rendering in `domain-events`, notification-rule event-kind matching in `domain-events`, assignment-rule event-kind matching in `domain-events` and notification-rule source payload-kind derivation in `domain-events`, direct user-notification source payload-kind derivation in `domain-events` and replayed queued-email source payload-kind derivation in `domain-events` and queued-email source payload-kind derivation in `domain-events` and assignment-rule source payload-kind derivation in `domain-events` and document-delivery source payload-kind derivation in `domain-events` and user-notification append-result event type derivation in `user-notification-events` and email-notification event type derivation in `email-notification-events` and user-account event type derivation in `user-account-events` and user-permission event type derivation in `user-permission-events` and notification-rule event type derivation in `notification-rule-events`, and recent slices that centralized print-settings event type derivation in `print-settings-events`, field-property event type derivation in `field-property-events`, role event type derivation in `role-events`, user-account status payload shaping in `user-account-events`, document create availability checks, AggregateCoordinator bulk failure document-name resolution, Durable Object bulk single-document command shaping, and bulk document selection result aggregation in `document-bulk-policy`, schema-before-hook validation issue ordering in core document hooks, document create and domain-command validation issue ordering in `document-command-policy`, document update validation issue ordering and aggregation, before-validate hook patch folding and validation hook issue collection in core document hooks, document after-commit hook execution, hook-error routing, tenant-aware DocType resolution in `document-tenant-policy` and shared-permission resolution in `document-access-policy` and report slices moved report document sorting, custom row sorting, bounded ordered export row selection, filter expression materialization and matching, filter coercion, filter-control shaping, order-control resolution, row/formula projection, chart result projection, and group projection into `report-policy`, while recent slices moved saved report fold, JSON definition payload codec, label normalization, owner projection, deterministic sorting, runtime report projection, payload-kind selection, and append-version helpers into `saved-report-events`, moved saved list filter fold, label normalization, owner projection, deterministic sorting, payload-kind selection, and append-version helpers into `saved-list-filter-events`, moved document delivery outbox fold, deterministic record identity, retry eligibility, record sorting, and selected-record projection into `document-delivery-outbox-events`, moved email notification outbox fold, deterministic message identity, active-claim matching, stale-claim detection, claim-id invariants, and append-result invariants into `email-notification-events`, moved user notification inbox fold, ordering, identity, subject shaping, append invariants, and replay invariants into `user-notification-events`, moved job schedule runtime definition command shaping into `job-schedule-policy` and runtime definition selectors alongside event creation/projection replay in `job-schedule-events`, extracted job history and retry decisions, routed `DocumentShareService` share-management access through `document-access-policy`, and added link-option candidate decisions, projection page scan decisions, projected-document read decisions, DocType action access decisions, shared-permission lookup decisions, unique-value owner lookup decisions, saved-event id lookup, starter Cron outbox drain coverage, starter realtime wiring coverage, moved application hook consumers onto the core document-hook contract, extracted document hook contracts and snapshotting into a core helper, centralized registration-order registry list snapshots, installed-app metadata snapshotting, named registry list snapshots behind one helper, guarded app dependency ordering with a frozen install-order result, the registry-options object boundary itself, registry-options composition collections, registry list collection boundaries, direct registry report and print metadata, raw app registry-options composition, direct registry DocType metadata, app manifest and direct registry hook metadata, app manifest DocType schema metadata, app manifest print metadata, app manifest report metadata, app manifest dashboard metadata, app manifest Kanban metadata, app manifest Calendar metadata, app manifest Web View metadata, app manifest Web Form metadata, app manifest Web Page metadata, app manifest workspace metadata, app manifest client script and data patch metadata, app manifest Website Theme metadata, app manifest Website Settings metadata, locked workspace and Website Settings role metadata snapshot coverage, guarded data patch rollback metadata, report roles and summary metadata, projection index metadata, domain command metadata, workflow metadata, permission rules, list-view filters, declared schema JSON defaults, and kept shrinking `FileService` by moving persisted file-name reads, scan-failure cleanup key planning, primary file object-key reads, delete-request event planning, multipart completion-start event planning, multipart operation state-set guards, scan-failed state classification, upload-pending state classification, multipart-completing state classification, delete-requested state classification, infected scan-result classification, infected scan-failure selection, optional scan-result patch planning, buffered-upload final document data shaping, multipart part manifest patch shaping, rendition manifest patch shaping, rendition put-command shaping, file object content identity, metadata patch guard validation, storage custom-metadata shaping, transform source shaping, downloaded transform command shaping, rendition download selection, delete object-key planning, upload size bounds, upload-expiry planning, transformability rejection, rendition reuse, generated-rendition reuse storage planning, generated-rendition reuse result shaping, generated-rendition reuse decision planning, generated-rendition reuse head-read planning, generated-rendition reuse object-existence shaping, generated-rendition completion manifest shaping, generated-rendition completion result shaping, generated-rendition failure cleanup key planning, generated-rendition failure cleanup planning, upload scan-failure cleanup planning, upload scan-failure decision planning, buffered upload create deduplication, scanner port guarding, CSV parser row-access hardening, CSV importable field resolution, data-patch single selection, data-patch runner single normalization, delete storage cleanup planning, delete finalization planning, generated-rendition failure recovery planning, generated-rendition failure-recovery suppression, generated-rendition failure manifest shaping, pending-generation reservation patch planning, pending-generation reservation document-command planning, latest-snapshot manifest update patch planning, snapshot-based rendition storage command planning, generated-rendition storage put-command planning, bulk failure result mapping, bulk metadata update failure mapping, bulk delete and metadata update entry command shaping, rendition failure result mapping, generated/downloaded rendition result shaping, upload/download/prepared-upload result shaping, bulk success and result shaping, bulk delete outcome aggregation, bulk metadata outcome aggregation, create permission/schema preflight, metadata update permission/state preflight, rendition generation permission preflight, multipart upload permission/state preflight, dashboard permission projection, dashboard result and direct-upload capability shaping, visible dashboard entry limiting, readable dashboard candidate shaping, readable dashboard entry projection, dashboard batch-limit planning, dashboard scan planning, dashboard system actor shaping, transform configuration preflight, storage capability preflight, dashboard capability/result shaping, dashboard pagination planning, cleanup failure suppression, upload document-data shaping, upload data command shaping, multipart abort delete command shaping, multipart abort planning, file document create command shaping, upload storage command shaping, buffered upload storage planning, buffered upload failure cleanup planning, multipart storage operation command shaping, multipart part upload planning, completed multipart object reuse/complete planning, completed multipart object read planning, completed multipart object head-read planning, direct-upload object head-read planning, downloaded object read planning, downloaded rendition object read planning, multipart completion snapshot selection, delete-requested snapshot selection, transformer command shaping, transformed file result shaping, upload completion command selection, upload completion planning, version, and execute-command shaping, multipart part record command and execute-command shaping, multipart completion-start command and execute-command shaping, delete-request command and execute-command shaping, delete command and execute-command shaping, rendition reservation command and execute-command shaping, rendition result command-name shaping, rendition manifest execute-command shaping, rendition manifest command shaping, metadata update command and execute-command shaping, buffered-upload create command shaping, buffered upload create planning, direct-upload create command shaping, direct upload reservation planning, multipart-upload create command shaping, multipart upload reservation planning, multipart upload reservation cleanup planning, command tenant and upload-default normalization, command option, attachment validation planning, attachment validation and option, privacy, transform-overlay resolution, transform-overlay document-read planning, transform-overlay object read planning, and transform-overlay shaping, missing-object error shaping, and delete permission/version preflight, object scanner target shaping, object scan planning, scanner target construction, scanner port guarding, CSV parser row-access hardening, CSV importable field resolution, data-patch single selection, data-patch runner single normalization, and result-status validation, dashboard query normalization and list-filter mapping, file metadata patch construction, file document payload construction, upload completion/failure patch shaping, multipart-completion state patches, file downloadability, direct-upload and multipart-upload state guards, multipart upload id reads, delete expected-version rules, file content length calculation, snapshot string reads, expected-version checks, scan-failure error shaping, dashboard row projection, dashboard limit normalization, direct-upload object matching, scan patch shaping, bulk file selection normalization, rendition manifest projection, deterministic rendition id planning, source matching, multipart manifest, part-size, reservation, completion, file naming, storage key, previewability, direct-upload bound, and rendition content-type planning into a pure file policy module with focused unit coverage.

## Status

This is not Frappe parity yet. Basic generated Desk list/form/report/print pages, metadata-defined workspaces and dashboards, event-sourced custom field overlays with parent, child table, and nested child-table custom fields, event-sourced field property overrides, generated HTTP and Desk administration plus runtime command/query/form/list/saved-filter/global-search/dashboard application and browser client helpers, field property override administration with command/query/Worker metadata composition, event-sourced runtime workflow definitions with HTTP/Desk/CLI administration and command/query/Worker metadata composition, event-sourced notification rules with HTTP/Desk/CLI administration, after-commit inbox delivery, Worker routing, and browser client helpers, generated Admin navigation, generated workflow transition actions, Frappe-style document duplication and cancelled-document amendment, selected-row bulk document deletion/lifecycle/workflow actions, permissioned document timelines with field diffs, activity feed entries, admin audit search and deleted recovery with browser client helpers, comments, assignments, tags, followers, event-sourced document shares with extracted event helper modeling and HTTP and Desk management routes, signed session, Cloudflare Access, generic OIDC actor resolution/account sync, and Okta/Auth0/Google Workspace OIDC claim presets, event-sourced role catalog administration with browser client helpers and remote CLI commands, optional catalog-backed account role validation, event-sourced user accounts with login/logout/me APIs and remote CLI commands, recovery-token flows, separate profile streams with profile preference fields, browser client helpers, remote CLI commands, and Desk account/profile administration, event-sourced user permissions with extracted event helper modeling, admin API/Desk/CLI management, and browser client helpers, durable user notification inboxes with HTTP/Desk read-dismiss workflows and browser client helpers, app manifest composition, declarative client script injection with a built-in Desk browser API, workspace metadata client helpers, dashboard client helpers, field-property and workflow-definition client helpers, document-history read helpers, document collaboration and saved-filter client helpers, print metadata/document/PDF/settings client helpers, report-builder client helpers, basic form event hooks, basic field controls, user feedback helpers, parsed realtime subscription helpers, saved user filters, bounded resource CSV exports, metadata-driven CSV import templates, command-bound CSV imports, generated Desk navigation/CSV import/bulk action helpers, event-sourced saved report definitions with HTTP and Desk report-builder APIs, Desk saved report builder filter operator/default/required/range presets, summary/group/arbitrary-depth formula-column field, literal, and nested formula operand/chart sort/limit/palette/value-label/axis-label controls, formula columns with numeric field, literal, and nested expression operands, bounded report group rows, metadata-configured form, list, global-search, and dashboard views with operator-aware flat and nested compound expression filters, generated arbitrary-depth point-and-click compound condition builders, metadata-aware ordering, and resolved filter-builder metadata, metadata-planned D1 migrations with retired projection-index drops, app-declared data patches/backfills with D1/in-memory applied and rollback journals plus admin API/Desk plan/rollback-plan/apply/rollback/retry/rollback-retry controls, browser client helpers, event-level document field unsets for rename/remove data patches, queue-backed apply, rollback, and rollback-retry enqueue APIs and Desk controls, and remote CLI status/plan/rollback-plan/apply/rollback/enqueue/rollback-enqueue/retry/rollback-retry/rollback-retry-enqueue operations, Cloudflare-native background job primitives with worker pools, durable execution history, retry administration, scheduler administration with event-sourced runtime schedule definition and enable/disable/pause-until/reset override controls, and job browser client helpers, R2-backed file attachments with HTTP metadata listing/update APIs, remote CLI uploads, downloads, preview downloads, transform downloads, and rendition downloads, direct upload reservation/finalization APIs, storage-level multipart upload primitives, browser chunked multipart upload helpers, scan hooks, record attachment panels, permissioned inline previews, bounded image transforms plus styled text-watermark and image-overlay source options through an injected transformer port, event-sourced persisted image renditions, bounded bulk delete/metadata updates, and a filtered Desk file manager, typed report filters, bounded report filter expression trees with Desk report-builder visual controls, report row ordering, report formula columns, report charts/exports/printouts with metadata-driven ordering/palette/value-label/axis-label/drilldown controls, custom print templates, reusable letterheads, event-sourced global print settings, field-level document merge planning plus server-side deterministic merge apply, Durable Object realtime tenant/DocType/document topics plus redacted user notifications, permissioned presence snapshots, realtime-updated Desk document presence panels with stale-document warnings, live field-edit intent, explicit shared-draft patch transport, generated shared-draft proposal apply actions with stale-version guards, and bounded durable replay, and starter CLI package-manager/dependency/app-registry/migration-file/seed-data-patch generation plus signed-session, Cloudflare Access, and OIDC starter auth-mode wiring and Access setup plan/apply commands exist, but conflict-free collaborative editing, setup automation beyond Cloudflare Access plus generic OIDC configuration, richer media handling such as video/audio processing and provider-backed overlay composition adapters, richer operator workflows, broader browser-side client APIs beyond the current metadata/auth/accounts/audit/roles/custom-fields/field-properties/workflows/notification-rules/user-permissions/data-patches/jobs/print/report-builder/resource/history/desk/search/dashboard/file/notification/report/collaboration/realtime surface, and a compatibility-sized test suite remain open. The current implementation is the event-sourced Cloudflare kernel needed to grow those surfaces without rewiring the foundation.
