...
- metadata-defined Web Forms with whitelisted public submission fields, published and login-required filtering, optional safe public routes and success URLs, generated name-based metadata routes and route-or-name themed HTML routes, route-aware Desk browser helpers, and remote CLI submit commands over the same document command boundary
- metadata-defined Web Views for Website-Generator-style public list/detail pages over permissioned projections, with canonical nested route/published fields, generated JSON routes and themed HTML routes, Desk browser helpers, and remote CLI reads
...
- metadata-defined Website Settings for manifest-owned site title, homepage route, and role-filtered navigation that can reference Web Pages, Web Forms, Web Views, or safe links and renders across public Web Page/Web View/Web Form HTML, with generated metadata API, `/` homepage redirect, and remote CLI reads
...
| Website Generator / Web View Pages | metadata-defined themed public list/detail pages with canonical nested route and published fields, rendered from permissioned `QueryService` projection reads without a second content store |
...
| Website Settings | metadata-defined site title, homepage route, and role-filtered navigation that references Web Pages, Web Forms, Web Views, or safe links, renders across public Web Page/Web View/Web Form HTML, and is exposed through metadata API, `/` redirect, and remote CLI reads |
...
Web View item routes are read from the configured DocType route field and must be safe canonical relative routes, so values like `docs/getting-started` render as nested public detail URLs while unsafe values are omitted from generated lists and cannot be resolved directly.
- `GET /page/:route`
- `GET /`
