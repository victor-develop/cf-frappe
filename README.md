...
Browser scripts can also build generated Desk navigation targets without duplicating route encoding: `window.cfFrappe.desk.listUrl(doctype, options)` returns the filtered/ordered list route, `window.cfFrappe.desk.newUrl(doctype)` returns `/desk/:doctype/new`, and `window.cfFrappe.desk.formUrl(doctype, name)` returns `/desk/:doctype/:name`.
