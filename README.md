...
The framework keeps presigning and scanning as injected adapters so applications can choose their own AWS SDK, signing service, queue/workflow-backed scanner, service binding, or local-development strategy without adding storage credentials or scanner credentials to core framework code. Downloads and permissioned inline previews share the same read and storage-state checks, with previews limited to conservative browser-safe content types. Image transforms and richer media workflows remain future adapters over the same file workflow boundaries.
