...
| Reports | metadata-defined report columns, filters, summaries, ordered/colored charts, API, and Desk pages |
...
HTTP clients can call `/api/report/Open%20Tasks/run?filter_priority=High`, or `/api/report/Open%20Tasks/export.csv?filter_priority=High` for a filtered CSV export. Desk renders the same report at `/desk/reports/Open%20Tasks` and exposes the CSV export beside the report filters. Report metadata can declare top-level summaries, grouped summaries, and charts backed by grouped summary metrics, all computed over the filtered result set before pagination. Chart metadata can order points by key, label, or value, cap the rendered point count, provide safe hex palettes, and hide value labels. This report slice is projection-backed and read-only; report builder workflows, advanced chart controls, and custom query adapters are future report layers over the same service boundary.
