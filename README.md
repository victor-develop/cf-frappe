...
- R2-backed file attachments with event-sourced `File` metadata, metadata updates, generated Desk record attachment panels, and file manager workflows
...
- installable `cf-frappe init` starter scaffold plus `cf-frappe install` package-manager, dependency metadata, app-registry wiring, migration generation, and remote data-patch operations for new Cloudflare apps
...
| File attachments | `File` DocType metadata plus R2 object storage, metadata updates, direct upload reservation/finalization, scan hooks, generated Desk record attachment panels, and file manager |
...
| Migrations/patches | metadata-planned D1 migrations plus app-declared data patches with applied journals, event-level field unsets for rename/remove backfills, and CLI-driven remote status/apply/enqueue |
...
Worker apps with registered patches expose `GET /api/data-patches` for an admin status dashboard, `POST /api/data-patches/apply` to claim and run unapplied patches, and `POST /api/data-patches/:id/apply` to run one registered patch. Bulk apply accepts `?limit=1` or a JSON body such as `{ "limit": 10, "patchIds": ["crm.customer_status_v1"] }`; limits apply to the next pending patches in registry order so operators can run reviewable batches without inverting app-declared patch order. The generated Desk also mounts `/desk/admin/data-patches` with the same journal-backed status and apply actions, and the Desk sidebar lists enabled admin tools such as data patches, roles, user permissions, jobs, and schedules. By default this uses D1's `cf_frappe_data_patches` journal and passes the normal runtime services as patch resources; apps can override `dataPatches.resources` in `createCloudFrappeWorker(...)` to give patches a narrower dependency surface.
...
File bytes live in a `FileStorage` port; file metadata is a regular event-sourced `File` document. On Cloudflare, `R2FileStorage` stores bytes in R2 while `DocumentService` records filename, object key, size, content type, attachment target, uploader, privacy, ETag, and optional scan status. When file support is enabled, HTTP clients can use `GET /api/files` for permission-aware metadata listing with attachment filters, `PATCH /api/files/:name` for event-sourced filename/privacy/attachment metadata updates, and `POST /api/files/direct-upload` plus `POST /api/files/:name/complete-upload` for direct browser upload reservations. Direct upload reservations create pending `File` metadata with a storage key and upload instructions; completion verifies object metadata and scanner results through ports before appending `FileDirectUploadCompleted` and making the file downloadable. Infected uploads append `FileScanFailed`, move the File to `scan_failed`, remove stored content when possible, and stay hidden from public readers. Desk adds `/desk/files` for file upload, readable file listing, metadata editing, content download, attachment filtering, and delete actions through the same `FileService` boundary. Generated Desk document forms also render an attachment panel for the current record, letting users upload, download, open the filtered file manager, and delete attached files without leaving the record page.
...
The framework keeps presigning and scanning as injected adapters so applications can choose their own AWS SDK, signing service, queue/workflow-backed scanner, service binding, or local-development strategy without adding storage credentials or scanner credentials to core framework code. Multipart chunking, image transforms, and richer file manager views remain future adapters over the same file workflow boundaries.
