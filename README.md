...
- metadata-configured list columns, default filters, saved user filters, system-field, pattern, presence, range, membership, scalar, and nested compound expression filters, filter-builder metadata, ordering, and page size
...
| App starter | `cf-frappe init` scaffold with Worker, D1, Durable Object, signed-session or Cloudflare Access account-sync wiring, `cf-frappe install` package-manager/dependency/registry wiring, and `cf-frappe data-patches` remote operations |
