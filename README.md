...
- metadata-defined Web Forms with whitelisted public submission fields, optional safe public routes and success URLs, generated name-based metadata routes and route-or-name themed HTML routes, Desk browser helpers, and remote CLI submit commands over the same document command boundary
...
| Web Forms | metadata-defined themed submission forms with optional safe public routes and success URLs that validate fields against DocType metadata, expose only declared writable fields, and create documents through `DocumentService` with name-based HTTP, route-or-name public HTML, browser-helper, and remote CLI surfaces |
...
Web Form JSON metadata and submit APIs use the Web Form name as the stable identifier. Public HTML routes resolve an optional safe `route` first, such as `/web-forms/task-intake`, and fall back to the encoded name for compatibility. When `successUrl` is set, public submissions render a themed success page with a safe Continue link.
