...
| Migrations/patches | metadata-planned D1 migrations plus app-declared data patches with applied journals and CLI-driven remote status/apply/enqueue |
...
HTTP clients can use `GET /api/resource/:doctype/:name/shares`, `POST /api/resource/:doctype/:name/shares`, and `DELETE /api/resource/:doctype/:name/shares/:userId` when document sharing is enabled in the resource API. Share events advance the document version, appear in timelines and audit search, and generate redacted per-user realtime/notification payloads for the affected user.
