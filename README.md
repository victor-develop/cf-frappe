...
- metadata-configured list columns, default filters, saved user filters, membership and scalar filter operators, filter-builder metadata, ordering, and page size
...
Resource list filters are parsed from query strings, validated against DocType metadata by `QueryService`, coerced to field types, and then executed by the active projection adapter. Scalar operators use one value; membership operators use repeated values such as `filter_priority__in=High&filter_priority__in=Medium` or `filter_priority__not_in=Low&filter_priority__not_in=Medium`. Saved filters persist the same normalized array value for `in` / `not_in`. Unknown fields, JSON fields, bad numeric/boolean values, empty membership arrays, and unsupported operators fail as `BAD_REQUEST`.
