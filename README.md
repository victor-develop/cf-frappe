...
- metadata-defined print formats with reusable letterheads, field sections, HTML templates with escaped substitutions, HTTP/browser/CLI metadata inspection, remote HTML/PDF downloads, and an optional Cloudflare Browser Rendering PDF adapter
...
Notification inboxes expose the same read/dismiss events as Desk, while rules are managed through the event-sourced automation stream that feeds durable inbox entries. Rules default to the `inbox` channel and can opt into `email` when the Worker wires an `EmailSender` port:
...
For Queue-backed retries, register the built-in delivery job and provide the same email notification service as a job resource:
