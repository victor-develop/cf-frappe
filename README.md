...
The framework keeps presigning and scanning as injected adapters so applications can choose their own AWS SDK, signing service, queue/workflow-backed scanner, service binding, or local-development strategy without adding storage credentials or scanner credentials to core framework code. Multipart chunking, image transforms, previews, and richer media workflows remain future adapters over the same file workflow boundaries.
