...
Queue consumers process each message independently: malformed messages and permanent failures are acknowledged, retryable failures use backoff, and job contexts carry an idempotency key. When an execution log is configured, cf-frappe stores running/succeeded/failed records, exposes an admin JSON dashboard at `/api/jobs`, and renders the same history in Desk at `/desk/admin/jobs`. For production, create the Queue with Wrangler and add producer/consumer bindings plus UTC Cron triggers in `wrangler.jsonc`.
