...
| Dashboards | metadata-defined Desk number cards backed by document counts and report summaries |
...
Dashboards are metadata registered beside DocTypes and reports. `defineDashboard(...)` cards can count readable documents with metadata-validated list filters or read an existing report summary through `ReportService`. `DashboardService` keeps those cards behind `QueryService` and `ReportService`, so DocType permissions, document shares, user-permission grants, report roles, report filters, and custom row-provider reports are reused instead of reimplemented. Worker apps expose `GET /api/meta/dashboards`, `GET /api/meta/dashboards/:dashboard`, and `GET /api/dashboard/:dashboard/run`; Desk renders `/desk/dashboards` and `/desk/dashboards/:dashboard` pages, and browser scripts can use `window.cfFrappe.dashboard.list/get/run` over the same routes.
