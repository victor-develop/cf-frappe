...
`CustomFieldService` stores tenant-specific DocType extensions as append-only `CustomFieldSaved` and `CustomFieldDisabled` events. It folds the current overlay for a DocType, validates every custom field against the base metadata, rejects base-field collisions, checks link/table targets against the registry, and can compose an effective DocType definition without mutating the base registry. System managers can manage the same overlay through `GET /api/custom-fields/:doctype`, `POST /api/custom-fields/:doctype`, `DELETE /api/custom-fields/:doctype/:field`, or the generated `/desk/admin/custom-fields` page. These adapters keep parsing and form coercion outside the service while preserving the event-sourced Customize Form boundary.
