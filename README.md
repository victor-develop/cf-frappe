...
| File attachments | `File` DocType metadata plus R2 object storage, metadata updates, direct upload reservation/finalization, generated Desk record attachment panels, and file manager |
...
File bytes live in a `FileStorage` port; file metadata is a regular event-sourced `File` document. On Cloudflare, `R2FileStorage` stores bytes in R2 while `DocumentService` records filename, object key, size, content type, attachment target, uploader, privacy, and ETag. When file support is enabled, HTTP clients can use `GET /api/files` for permission-aware metadata listing with attachment filters, `PATCH /api/files/:name` for event-sourced filename/privacy/attachment metadata updates, and `POST /api/files/direct-upload` plus `POST /api/files/:name/complete-upload` for direct browser upload reservations. Direct upload reservations create pending `File` metadata with a storage key and upload instructions; completion verifies object metadata through the storage port before appending `FileDirectUploadCompleted` and making the file downloadable. Desk adds `/desk/files` for file upload, readable file listing, metadata editing, content download, attachment filtering, and delete actions through the same `FileService` boundary. Generated Desk document forms also render an attachment panel for the current record, letting users upload, download, open the filtered file manager, and delete attached files without leaving the record page.
...
The framework keeps the presigner as an injected adapter so applications can choose their own AWS SDK, signing service, or local-development strategy without adding storage credentials to core framework code. Multipart chunking, virus scanning hooks, image transforms, and richer file manager views remain future adapters over the same `FileStorage` boundary.
