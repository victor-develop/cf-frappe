...
Transactional email delivery stays behind a port so notification rules remain metadata and event-stream state rather than provider code. On Cloudflare, bind Email Sending with `send_email`, enable the sending domain in Cloudflare Email Service, and wire the Durable Object coordinator to `CloudflareEmailSender`:
