...
- R2-backed file attachments with event-sourced `File` metadata, metadata updates, generated Desk record attachment panels, and filtered file manager workflows
...
Generated list pages also expose permission-aware selected-row deletion for draft and cancelled documents. `POST /api/resource/:doctype/delete` accepts bounded document selections with optional `expectedVersion` values and returns per-document deleted/failed outcomes, while `/desk/:doctype/bulk-delete` submits the same command from detached row checkboxes. Both surfaces call `DocumentService.bulkDelete(...)`, which validates the batch before writing and then reuses the single-document event-sourced delete path for permissions, user permissions, docstatus guards, hooks, projections, and auditability.
...
The framework keeps presigning and scanning as injected adapters so applications can choose their own AWS SDK, signing service, queue/workflow-backed scanner, service binding, or local-development strategy without adding storage credentials or scanner credentials to core framework code. Multipart chunking, image transforms, previews, and bulk metadata workflows remain future adapters over the same file workflow boundaries.
