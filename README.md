...
| App starter | `cf-frappe init` scaffold with Worker, D1, Durable Object, signed-session or Cloudflare Access account-sync wiring, `cf-frappe access` setup automation, `cf-frappe install` package-manager/dependency/registry wiring, and `cf-frappe data-patches` remote operations |
