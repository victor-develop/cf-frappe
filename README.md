...
- metadata-defined read-only Calendar views over permissioned date/datetime projections with HTTP, Desk, browser, and remote CLI reads
...
- metadata-defined Web Forms with whitelisted public submission fields, published filtering, optional safe public routes and success URLs, generated name-based metadata routes and route-or-name themed HTML routes, Desk browser helpers, and remote CLI submit commands over the same document command boundary
- metadata-defined Web Views for Website-Generator-style public list/detail pages over permissioned projections, with route/published fields, generated JSON routes and themed HTML routes, Desk browser helpers, and remote CLI reads
...
- metadata-defined Website Settings for manifest-owned site title, homepage route, and role-filtered navigation rendered across public Web Page/Web View/Web Form HTML, with generated metadata API, `/` homepage redirect, and remote CLI reads
...
| Calendar views | metadata-defined read-only date/datetime event projections through permissioned `QueryService` list reads, with HTTP, Desk, browser, workspace shortcut, and remote CLI metadata/run surfaces |
...
| Web Forms | metadata-defined themed submission forms with published filtering plus optional safe public routes and success URLs that validate fields against DocType metadata, expose only declared writable fields, and create documents through `DocumentService` with name-based HTTP, route-or-name public HTML, browser-helper, and remote CLI surfaces |
| Website Generator / Web View Pages | metadata-defined themed public list/detail pages with route and published fields, rendered from permissioned `QueryService` projection reads without a second content store |
...
| Website Settings | metadata-defined site title, homepage route, and role-filtered navigation that references Web Pages or safe links, renders across public Web Page/Web View/Web Form HTML, and is exposed through metadata API, `/` redirect, and remote CLI reads |
...
export const Website = defineWebsiteSettings({
  ...
  navItems: [
    ...
    { name: "task-intake", label: "Task Intake", href: "/web-forms/task-intake" }
  ]
});
...
Web Form JSON metadata and submit APIs use the Web Form name as the stable identifier. Public HTML routes resolve an optional safe `route` first, such as `/web-forms/task-intake`, and fall back to the encoded name for compatibility. Forms with `published: false` are hidden from public metadata, listing, and submission routes for non-admin actors. When `successUrl` is set, public submissions render a themed success page with a safe Continue link.
