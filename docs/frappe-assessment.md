...
| Session auth | sessions | signed cookie actor resolver, optional event-sourced account/login/recovery routes, and env-backed Worker composition |
...
- Auth providers remain adapter seams; signed cookie actor resolution, event-sourced role catalogs, optional catalog-backed account role validation, event-sourced account/login/recovery routes, generic password reset and email verification token flows, and basic Desk account administration exist, but provider-specific integrations and richer profile workflows are still future work.
