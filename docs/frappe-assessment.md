...
| Reports | Report DocType/query reports | metadata-defined report columns, filters, summaries, charts, API, and Desk pages |
...
- Generated Desk UI now covers basic list/form/report/print pages, model-defined form sections and field order, list columns, default list filters, saved user filters, filter controls, link-field select controls, child table row grids, submit/cancel lifecycle actions, document timelines/comments/assignments, report summaries/charts, and page size. Advanced filter builders, report builder, custom print templates, letterheads, richer admin tools, richer chart controls, and client scripting are not implemented.
