...
| Custom fields | Custom Field / Customize Form | event-sourced tenant custom field overlays folded into base DocType metadata by `CustomFieldService`, with generated HTTP and Desk administration |
...
| REST resources | `/api/resource/:doctype` | generated Hono routes with metadata-configured list views, operator-aware filters, resolved filter-builder metadata, and bounded selected-document bulk delete/lifecycle/workflow actions |
| Desk views | list/form views from metadata | generated server-rendered `/desk` list/forms with model-defined form sections, columns, list columns, saved filters, operator-aware filters, filter-builder metadata, permission-aware selected-row bulk delete/lifecycle/workflow actions, and page size |
