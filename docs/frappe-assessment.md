...
- Auth providers remain adapter seams; signed cookie actor resolution, event-sourced role catalogs, optional catalog-backed account role validation, event-sourced account/login routes, and basic Desk account administration exist, but password reset, email verification, provider-specific integrations, and richer profile workflows are still future work.
