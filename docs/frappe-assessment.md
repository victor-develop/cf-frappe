...
- Auth providers remain adapter seams; signed cookie actor resolution, event-sourced role catalogs, event-sourced account/login routes, and basic Desk account administration exist, but password reset, email verification, provider-specific integrations, catalog-backed account role validation, and richer profile workflows are still future work.
