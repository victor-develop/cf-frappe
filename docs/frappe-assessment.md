...
| App starter | bench/new app setup | initial `cf-frappe init` scaffold with Worker, D1, Durable Object, signed-session wiring, and `cf-frappe install` dependency/app-registry wiring |
...
- DocType metadata now plans and applies D1 projection-index migrations with a checksum journal, app manifests compose DocTypes/hooks/reports/print formats, and the CLI can save app dependency metadata while wiring app modules into generated registries, but destructive/renaming migrations, package-manager execution, lockfile updates, migration generation after app changes, data backfills, and auth-provider setup are still future work.
...
- The CLI now creates a Cloudflare-ready starter app and can save app package metadata while wiring app modules into the generated registry, but it does not yet execute package managers, update lockfiles, generate model migrations after app changes, run data backfills, or set up auth providers.
