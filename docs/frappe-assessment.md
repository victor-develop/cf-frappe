...
| Migrations | patches and schema migrations | D1 migration plans, rendered SQL bundles, applied checksum journal, and app-declared data patches with applied journals plus API/Desk apply controls with optional bounds |
| Workflow | Workflow DocType | metadata transitions and transition events |
| Background jobs | scheduler and queue workers | `JobRegistry`, queue dispatch/consume, Cron mapping, event-sourced runtime schedule overrides, D1-backed execution history, and failed-job retry admin |
| File attachments | File DocType plus file store | `File` metadata DocType plus R2/in-memory `FileStorage`, generated record attachment panels, and Desk file manager |
| Realtime notifications | `publish_realtime`/Socket.IO | document commit events over Durable Object WebSocket topics with tenant, DocType, and document fan-out plus redacted user-recipient notifications, basic presence snapshots, and bounded durable replay |
...
- Generated Desk UI now covers basic list/form/report/print pages, generated Admin navigation, model-defined form sections and field order, list columns, default list filters, saved user filters, operator-aware filter controls, resolved filter-builder metadata, link-field select controls, child table row grids, submit/cancel lifecycle actions, metadata workflow transition actions, document timelines with field diffs/comments/assignments/tags/followers, role administration, user-permission administration, user-account administration, typed report filters, report row ordering, event-sourced saved report definitions with HTTP and Desk report-builder APIs, report summaries/charts/exports with metadata-driven ordering/palette/value-label controls, custom print templates, reusable letterheads, same-origin client script injection with a built-in browser API, basic form event hooks, field controls, user feedback helpers, parsed realtime subscriptions, and page size. Richer visual filter builders, richer report-builder controls, broader admin tools, advanced chart controls, and broader browser-side client APIs are not implemented.
- DocType metadata now plans and applies D1 projection-index migrations with retired-index drops and a checksum journal, app manifests compose DocTypes/hooks/reports/print formats/data patches, and the CLI can save app dependency metadata, run detected package managers for lockfile/node_modules updates, wire app modules into generated registries, and generate reviewable D1 migration files after versioned DocType index changes. Basic app-declared data patches/backfills have D1 and in-memory applied journals plus admin API/Desk status and apply routes with bounded and single-patch execution, and queue-backed apply jobs let admins enqueue validated patch plans through Cloudflare Queues. Event-level destructive/renaming data migrations, richer patch workflow controls, and auth-provider setup are still future work.
- Background jobs now have Queue/Cron support plus schedule admin, event-sourced runtime enable/disable overrides, D1-backed execution history with API/Desk admin dashboards, manual schedule dispatch, and failed-execution retry actions, but worker pools and richer scheduler editing are not implemented.
...
- Realtime notifications now have Durable Object WebSocket tenant, DocType, and document topics plus redacted user-recipient notifications, basic presence snapshots, and bounded durable replay with Desk client helpers, but richer presence, richer Desk consumption, and collaboration-oriented realtime are not implemented.
- Auth providers remain adapter seams; signed cookie and Cloudflare Access actor resolution, event-sourced role catalogs, optional catalog-backed account role validation, event-sourced account/profile/login/recovery routes, generic password reset and email verification token flows, self/admin profile updates, and basic Desk account/profile administration exist, but additional provider integrations and richer profile preferences/provider sync are still future work.
...
- File storage now has basic R2-backed attachments plus permission-aware HTTP metadata listing/update, generated record attachment panels, and Desk upload/list/metadata/download/delete workflows, but multipart chunking, presigned browser uploads, virus scanning hooks, image transforms, and richer file manager views are not implemented.
- The CLI now creates a Cloudflare-ready starter app and can save app package metadata, run detected package managers to update lockfiles and `node_modules`, wire app modules into the generated registry, and generate reviewable D1 migration files after versioned DocType index changes. Worker apps can inspect/apply app-declared patches through admin API and Desk routes, but the CLI does not yet drive remote patch execution or auth-provider setup.
