...
| Migrations | patches and schema migrations | D1 migration plans, rendered SQL bundles, applied checksum journal, and app-declared data patches with applied journals plus admin apply APIs with optional bounds |
...
- DocType metadata now plans and applies D1 projection-index migrations with a checksum journal, app manifests compose DocTypes/hooks/reports/print formats/data patches, and the CLI can save app dependency metadata, run detected package managers for lockfile/node_modules updates, wire app modules into generated registries, and generate reviewable D1 migration files after versioned DocType index changes. Basic app-declared data patches/backfills have D1 and in-memory applied journals plus admin JSON status/apply routes with bounded and single-patch execution, but destructive/renaming migrations, queued/async patch orchestration, and auth-provider setup are still future work.
