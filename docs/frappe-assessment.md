...
| Migrations | patches and schema migrations | D1 migration plans, rendered SQL bundles, applied checksum journal, app-declared data patches with applied journals, event-level field unsets for rename/remove backfills, and API/Desk/CLI status, apply, and enqueue controls with optional bounds |
...
- DocType metadata now plans and applies D1 projection-index migrations with retired-index drops and a checksum journal, app manifests compose DocTypes/hooks/reports/print formats/data patches, and the CLI can save app dependency metadata, run detected package managers for lockfile/node_modules updates, wire app modules into generated registries, generate reviewable D1 migration files after versioned DocType index changes, and drive remote data-patch status/apply/enqueue operations through the same admin API as Desk. Basic app-declared data patches/backfills have D1 and in-memory applied journals plus admin API/Desk/CLI status and apply routes with bounded and single-patch execution, event-level document field unsets let rename/remove backfills stay append-only, and queue-backed apply jobs let admins enqueue validated patch plans through Cloudflare Queues. Richer patch workflow controls and auth-provider setup are still future work.
