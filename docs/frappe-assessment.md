...
- DocType metadata now plans and applies D1 projection-index migrations with a checksum journal, but destructive/renaming migrations, data backfills, and richer CLI-driven app installation are still future work.
...
- The CLI now creates a Cloudflare-ready starter app, but it does not yet manage installed apps, generated model migrations, data backfills, or auth-provider setup.
