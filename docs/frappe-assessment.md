...
- DocType metadata now plans and applies D1 projection-index migrations with retired-index drops and a checksum journal, app manifests compose DocTypes/hooks/reports/print formats/data patches, and the CLI can save app dependency metadata, run detected package managers for lockfile/node_modules updates, wire app modules into generated registries, and generate reviewable D1 migration files after versioned DocType index changes. Basic app-declared data patches/backfills have D1 and in-memory applied journals plus admin API/Desk status and apply routes with bounded and single-patch execution, but event-level destructive/renaming data migrations, queued/async patch orchestration, and auth-provider setup are still future work.
