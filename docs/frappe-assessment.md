...
| Session auth | sessions | signed cookie actor resolver, optional event-sourced account/profile/login/recovery routes, and env-backed Worker composition |
...
- DocType metadata now plans and applies D1 projection-index migrations with a checksum journal, app manifests compose DocTypes/hooks/reports/print formats/data patches, and the CLI can save app dependency metadata, run detected package managers for lockfile/node_modules updates, wire app modules into generated registries, and generate reviewable D1 migration files after versioned DocType index changes. Basic app-declared data patches/backfills have D1 and in-memory applied journals plus admin API/Desk status and apply routes with bounded and single-patch execution, but destructive/renaming migrations, queued/async patch orchestration, and auth-provider setup are still future work.
...
- Auth providers remain adapter seams; signed cookie actor resolution, event-sourced role catalogs, optional catalog-backed account role validation, event-sourced account/profile/login/recovery routes, generic password reset and email verification token flows, self/admin profile updates, and basic Desk account/profile administration exist, but provider-specific integrations and richer profile preferences/provider sync are still future work.
