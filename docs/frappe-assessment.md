...
| App starter | bench/new app setup | initial `cf-frappe init` scaffold with Worker, D1, Durable Object, signed-session wiring, and `cf-frappe install` app-registry wiring |
...
- DocType metadata now plans and applies D1 projection-index migrations with a checksum journal, app manifests compose DocTypes/hooks/reports/print formats, and the CLI can wire app modules into generated registries, but destructive/renaming migrations, package installation, migration generation after app changes, data backfills, and auth-provider setup are still future work.
...
- The CLI now creates a Cloudflare-ready starter app and can wire installed app modules into the generated registry, but it does not yet install third-party app packages, generate model migrations after app changes, run data backfills, or set up auth providers.
