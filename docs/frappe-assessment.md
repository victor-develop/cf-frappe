...
| Document lifecycle | docstatus draft/submitted/cancelled plus Duplicate | first-class submit/cancel events with command-side status guards plus duplication through a source-read/target-create `DocumentCreated` path |
| Lifecycle behavior | hooks/controllers | registry hooks: `beforeValidate`, `validate`, `afterCommit` |
| REST resources | `/api/resource/:doctype` | generated Hono routes with metadata-configured list views, operator-aware filters, resolved filter-builder metadata, duplicate actions, and bounded selected-document bulk delete/lifecycle/workflow actions |
| Desk views | list/form views from metadata | generated server-rendered `/desk` list/forms with model-defined form sections, columns, list columns, saved filters, operator-aware filters, filter-builder metadata, duplicate actions, permission-aware selected-row bulk delete/lifecycle/workflow actions, and page size |
...
- File storage now has basic R2-backed attachments plus permission-aware HTTP metadata listing/update, direct browser upload reservation/finalization APIs behind the `FileStorage` signer boundary, event-sourced file scan hooks, generated record attachment panels, bounded selected-file metadata/delete outcomes, and Desk upload/list/filter/bulk-delete/bulk-metadata/metadata/download/delete workflows. Multipart chunking, image transforms, previews, and richer media workflows are not implemented.
