...
| Audit trail | document versioning/activity | permissioned timelines with field diffs, comments, and assignments projected from append-only events plus model-declared domain commands |
...
- Document history now exposes permissioned timeline entries with bounded event-sourced field-level old/new diffs, comment events, and assignment events from the event stream, but deleted-document audit recovery, email/activity feed entries, and admin-only audit search are not implemented.
