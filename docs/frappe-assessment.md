...
| Migrations | patches and schema migrations | D1 migration plans, rendered SQL bundles, applied checksum journal, and app-declared data patches with applied journals |
...
- The CLI now creates a Cloudflare-ready starter app and can save app package metadata, run detected package managers to update lockfiles and `node_modules`, wire app modules into the generated registry, and generate reviewable D1 migration files after versioned DocType index changes, but it does not yet orchestrate patch execution or set up auth providers.
