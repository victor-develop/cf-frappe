...
- Extracted job schedule admin access, schedule visibility, manual-dispatch eligibility, override eligibility, override write/no-op decisions, and runtime definition save/delete eligibility from `JobScheduleService` into `src/application/job-schedule-policy.ts`, keeping runner, registry, and event-store orchestration in the service while covering schedule decisions behind focused unit tests.
