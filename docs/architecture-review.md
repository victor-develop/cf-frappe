...
- File workflows now delegate reusable-rendition selection, pending reservation patch planning, latest-snapshot manifest update patch planning, snapshot-based rendition storage command planning, bulk failure result mapping, rendition failure result mapping, create permission/schema preflight, metadata update permission/state preflight, and delete permission/version preflight to pure policy functions while `FileService` keeps I/O orchestration.
