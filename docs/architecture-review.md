...
- Report workflows now delegate filter expression materialization and matching, filter coercion, filter-control shaping, order-control resolution, row and formula projection, summary aggregation, group projection, chart result shaping, point sorting, drilldown query shaping, and row-cap policy to `report-policy`, keeping `ReportService` focused on metadata validation, permissioned reads, filtering, ordering, export orchestration, and custom row-provider I/O.
