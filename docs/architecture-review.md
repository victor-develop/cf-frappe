...
- Extracted Desk file service availability decisions into file policy so Desk file routes keep actor resolution, dashboard query parsing, upload/download/delete orchestration, and failure rendering.
- Extracted Desk notification service availability decisions into user-notification policy so Desk notification routes keep actor resolution, inbox query parsing, read/dismiss orchestration, redirects, and page rendering.
...
- Extracted configured/runtime job-schedule lookup decisions into job-schedule policy so JobScheduleService keeps configured schedule scanning, runtime definition replay, summary projection, and framework error mapping.
...
- Re-ran a standalone architecture review on 2026-06-30 at commit `bfc52ced`; verdict remained fail. The reviewer confirmed the event payload union, D1 event writer, outbox Worker-path coverage, and test parity had improved, but flagged remaining blockers: `DocumentService` command orchestration breadth and incomplete multi-stream atomicity for naming-series allocation and unique-value release events.
- Moved naming-series allocation into the same `DocumentStore.commitBatch` as document creation, preserving expected-version conflict retries without consuming a series number when the create fails.
- Moved unique-value release writes for update and delete commands into the same `DocumentStore.commitBatch` as the document event, removing the old suppressible post-commit release path.
- Extracted atomic auxiliary batch entry and projection shaping into `src/application/document-atomic-commit-policy.ts`, keeping `DocumentService` focused on command I/O orchestration while the multi-stream commit composition policy owns naming-series, unique-reservation, unique-release, and document batch assembly.
- Added focused regression tests proving failed series document creates leave the naming-series stream untouched and failed update and delete unique-release batches leave both the document and unique reservation unchanged.
- Re-ran a fresh standalone architecture review after the atomic command boundary changes; verdict passed with no blocking architecture findings. Non-blocking follow-ups were addressed by declaring the Node `>=22` engine required by Wrangler and adding direct delete release atomicity coverage.
- Appended the default list sort column (`updated_at`) to metadata-planned D1 projection indexes so filtered list reads stop falling back to a temporary B-tree for `ORDER BY updated_at`. The index key became `(tenant_id, doctype, ...declared fields, updated_at)` in place of the previous filter-only shape. This changes the `doctype_<slug>_v<n>_indexes` checksum, so a database that already applied the old shape must reset or bump `doctype.version`; the framework is pre-1.0 with no production installations, so no compatibility shim was kept for the superseded shape. DocType authors keep declaring `indexes` over declared fields only. Recorded the ordering and predicate shapes the index cannot serve (`in`, ranges, non-default `orderBy`) in `docs/projection-indexes.md` rather than implying the fix is general.
- Declined to gather D1 planner statistics anywhere, including in migrations, and shipped `readD1Statistics` / `clearD1Statistics` / `analyzeD1Statistics` as diagnostics instead. Measured on a real local D1 with bound parameters against the generated projection indexes: the partial indexes are already chosen without any statistics, because D1 sends SQL and parameters together so the planner can satisfy the index's `WHERE doctype = '...'` predicate from the bound value. Statistics gathered with representative data improved no plan and pushed `in` filters onto a partial index plus a sort they did not previously need; statistics gathered while the table was empty cost the plain unfiltered list its index. The remedy for a database carrying such statistics is to drop them, which is why the clear operation exists and why nothing gathers them automatically.
