...
- Extracted job schedule admin access, schedule visibility, manual-dispatch eligibility, override eligibility, override write/no-op decisions, schedule summary shaping, runtime definition save/delete eligibility, event creation, and event projection from `JobScheduleService` into focused job schedule policy/event modules, keeping runner, registry, and event-store orchestration in the service while covering schedule decisions, event factories, and projection replay behind focused unit tests.
