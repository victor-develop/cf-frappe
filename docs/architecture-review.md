...
- File rendition generation now delegates reusable-rendition selection, pending reservation patch planning, latest-snapshot manifest update patch planning, snapshot-based rendition storage command planning, and bulk failure result mapping to pure policy functions while `FileService` keeps I/O orchestration.
