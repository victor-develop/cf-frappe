...
- Extracted job schedule admin access, schedule visibility, and manual-dispatch eligibility from `JobScheduleService` into `src/application/job-schedule-policy.ts`, keeping runner, registry, and event-store orchestration in the service while covering schedule decisions behind focused unit tests.
